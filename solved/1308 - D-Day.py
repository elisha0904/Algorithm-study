...
# 여러모로 고민해보다가 datetime을 사용하면 쉽게 풀릴 것 같아 활용.
import datetime

x = list(map(int, input().split()))
y = list(map(int, input().split()))

start = datetime.date(x[0], x[1], x[2])
today = datetime.date(y[0], y[1], y[2])

dday = today - start
result = list(str(dday).split())

# 위의 코드를 활용하여 1000년이 며칠인지 계산한 뒤 1000년 이상이면 gg 출력
if int(result[0]) > 365242:
    print('gg')
else:
    print(f'D-{result[0]}')
